...
extern void mov_volitivo_asm(MATRIX last_x, int np, int d, int padding_d, type stepvol, VECTOR baricentro, type direzione, VECTOR r);  // PERNA MANGIONE
...
void fss(params* input){
	...
	while (it < input->iter){
		...
		mov_volitivo_asm(input->x, input->np, input->d, input->padding_d, input->stepvol, baricentro, (peso_tot_old < peso_tot_cur) ? -1.0 : 1.0, &(input->r[ind_r]));
