...
void fss(params* input){
	...
	if(!input->silent) ("f min iniziale = %f\n", f_min);
	while (it < input->iter){
...
void mov_individuali(params* input, VECTOR deltaf, MATRIX deltax, MATRIX y, type* mindeltaf, VECTOR f_cur, int* ind_r){
  ...
  for(int pesce = 0; pesce < n_pesci; pesce++){ // numero pesci
	...
	for(int coordinata = 0; coordinata < n_coordinate; coordinata++){ // coordinate pesce
	  ...
	  *ind_r = *ind_r + 1;
      ...
      deltax[(n_coordinate+padding_d)*pesce+coordinata] = coord_j_pesce_i - val_coordinata; // aggiorno direttamente il delta x
    }  // for coordinate
    type val_f_pesce_cur_posy = exp(y_quadro) + y_quadro - c_per_y;
	
	if(val_f_pesce_cur_posy >= f_cur[pesce]){ // la posizione non è migliore
      deltaf[pesce] = 0.0; 
      /* non  è necessario azzerrare le coordinate deltaX del pesce
	     poichè basta controllare il suo deltaf prima di accedervi. */
...
void calcola_f_pesce(params* input, int pesce, type* ret){
	...
	for(int i = 0; i < n_coordinate; i++){ // coordinate pesce
      ...
      val_i = input->x[pesce*(n_coordinate+padding_d)+i]; //valore coordinata
...
void mov_istintivo(params* input, VECTOR deltaf, VECTOR deltax, VECTOR I){
	...
	for(int i = 0; i < n_pesci; i++){
		for(int j = 0; j < n_coordinate; j++){
			input->x[i*(n_coordinate+padding_d)+j] += I[j];
		}
	}
}
...
void mov_volitivo(params* input,  VECTOR baricentro, type* peso_tot_old, type* peso_tot_cur, int* ind_r){
	...
	for(int i = 0; i < n_pesci; i++){
		...
		*ind_r = *ind_r + 1;
		for(int j = 0; j < n_coordinate; j++){
			input->x[i*(n_coordinate+padding_d)+j] += (direzione)*(input->stepvol)*(rand)*((input->x[i*(n_coordinate+padding_d)+j]-baricentro[j])/dist);
		}
	}
